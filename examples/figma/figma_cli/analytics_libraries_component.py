...
app = typer.Typer(no_args_is_help=True, help="Manage analytics libraries component")


@app.command("commands", short_help="Display commands tree for component sub-commands")
def show_commands(
    display: _a.TreeDisplayOption = _a.TreeDisplay.HELP,
    depth: _a.MaxDepthOption = 5,
    search: _a.TreeSearchOption = None,
) -> None:
    """Show component sub-commands.

    The '*' denotes a sub-command with other sub-commands, but no direct actions.
    """
    path = Path(__file__).parent / "tree.yaml"
    _t.tree(path.as_posix(), "analytics_libraries_component", display, depth, search)
    return
...
@app.command("actions", short_help="Get library analytics component action data.")
def get_library_analytics_component_actions(
    ...
    '''
    # handler for getLibraryAnalyticsComponentActions: GET /v1/analytics/libraries/{file_key}/component/actions
    _l.init_logging(_log_level)
    headers = _r.request_headers(_api_key)
    url = _r.create_url(_api_host, "v1/analytics/libraries", file_key, "component/actions")
    missing = []
    if _api_key is None:
...
@app.command("usages", short_help="Get library analytics component usage data.")
def get_library_analytics_component_usages(
    ...
    '''
    # handler for getLibraryAnalyticsComponentUsages: GET /v1/analytics/libraries/{file_key}/component/usages
    _l.init_logging(_log_level)
    headers = _r.request_headers(_api_key)
    url = _r.create_url(_api_host, "v1/analytics/libraries", file_key, "component/usages")
    missing = []
    if _api_key is None:
        missing.append("--api-key")
    if group_by is None:
