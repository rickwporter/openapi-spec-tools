...
app = typer.Typer(no_args_is_help=True, help="Manage analytics libraries style")


@app.command("commands", short_help="Display commands tree for style sub-commands")
def show_commands(
    display: _a.TreeDisplayOption = _a.TreeDisplay.HELP,
    depth: _a.MaxDepthOption = 5,
    search: _a.TreeSearchOption = None,
) -> None:
    """Show style sub-commands.

    The '*' denotes a sub-command with other sub-commands, but no direct actions.
    """
    path = Path(__file__).parent / "tree.yaml"
    _t.tree(path.as_posix(), "analytics_libraries_style", display, depth, search)
    return
...
def get_library_analytics_style_usages(
    ...
    '''
    # handler for getLibraryAnalyticsStyleUsages: GET /v1/analytics/libraries/{file_key}/style/usages
    _l.init_logging(_log_level)
    headers = _r.request_headers(_api_key)
    url = _r.create_url(_api_host, "v1/analytics/libraries", file_key, "style/usages")
    missing = []
    if _api_key is None:
        missing.append("--api-key")
    if group_by is None:
