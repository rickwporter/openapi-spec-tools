...
app = typer.Typer(no_args_is_help=True, help="Manage files meta")


@app.command("commands", short_help="Display commands tree for meta sub-commands")
def show_commands(
    display: _a.TreeDisplayOption = _a.TreeDisplay.HELP,
    depth: _a.MaxDepthOption = 5,
    search: _a.TreeSearchOption = None,
) -> None:
    """Show meta sub-commands.

    The '*' denotes a sub-command with other sub-commands, but no direct actions.
    """
    path = Path(__file__).parent / "tree.yaml"
    _t.tree(path.as_posix(), "files_meta", display, depth, search)
    return


@app.command("show", short_help="Get file metadata")
def get_file_meta(
