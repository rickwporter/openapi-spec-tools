...
app = typer.Typer(no_args_is_help=True, help="Manage analytics libraries component usages")


@app.command("commands", short_help="Display commands tree for usages sub-commands")
def show_commands(
    display: _a.TreeDisplayOption = _a.TreeDisplay.HELP,
    depth: _a.MaxDepthOption = 5,
    search: _a.TreeSearchOption = None,
) -> None:
    """Show usages sub-commands.

    The '*' denotes a sub-command with other sub-commands, but no direct actions.
    """
    path = Path(__file__).parent / "tree.yaml"
    _t.tree(path.as_posix(), "analytics_libraries_component_usages", display, depth, search)
    return
...
@app.command("show", short_help="Get library analytics component usage data.")
