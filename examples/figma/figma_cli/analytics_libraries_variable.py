...
app = typer.Typer(no_args_is_help=True, help="Manage analytics libraries variable")


@app.command("commands", short_help="Display commands tree for variable sub-commands")
def show_commands(
    display: _a.TreeDisplayOption = _a.TreeDisplay.HELP,
    depth: _a.MaxDepthOption = 5,
    search: _a.TreeSearchOption = None,
) -> None:
    """Show variable sub-commands.

    The '*' denotes a sub-command with other sub-commands, but no direct actions.
    """
    path = Path(__file__).parent / "tree.yaml"
    _t.tree(path.as_posix(), "analytics_libraries_variable", display, depth, search)
    return
...
@app.command("actions", short_help="Get library analytics variable action data.")
def get_library_analytics_variable_actions(
    ...
    '''
    # handler for getLibraryAnalyticsVariableActions: GET /v1/analytics/libraries/{file_key}/variable/actions
    _l.init_logging(_log_level)
    headers = _r.request_headers(_api_key)
    url = _r.create_url(_api_host, "v1/analytics/libraries", file_key, "variable/actions")
    missing = []
    if _api_key is None:
...
@app.command("usages", short_help="Get library analytics variable usage data.")
def get_library_analytics_variable_usages(
    ...
    '''
    # handler for getLibraryAnalyticsVariableUsages: GET /v1/analytics/libraries/{file_key}/variable/usages
    _l.init_logging(_log_level)
    headers = _r.request_headers(_api_key)
    url = _r.create_url(_api_host, "v1/analytics/libraries", file_key, "variable/usages")
    missing = []
    if _api_key is None:
        missing.append("--api-key")
    if group_by is None:
