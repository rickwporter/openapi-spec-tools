...
def test_op_query_arguments():
    ...
    assert 'more: Annotated[bool, typer.Option(hidden=True)] = False' in text
    assert (
    ...
    assert (
        'enum_with_default: Annotated[EnumWithDefault, typer.Option(case_sensitive=False)] = "TheOtherThing"'
        in text
    )
    assert (
        'str_enum_with_int_values: Annotated[StrEnumWithIntValues, typer.Option(case_sensitive=False)] = "1"'
        in text
    )
    assert (
    ...
    assert (
        'addr_zip_code: Annotated[Optional[str], typer.Option(show_default=False)] = None'
        in text
    )
    assert (
    ...
    assert (
        'crazy_enum: Annotated[CrazyEnum, typer.Option(case_sensitive=False)] = "1.0"'
        in text
    )
    assert (
        'list_enum_def_list: Annotated[list[ListEnumDefList], typer.Option(case_sensitive=False)] '
        "= ['1', '8']"
        in text
    )
    assert (
        'list_int_enum: Annotated[list[ListIntEnum], typer.Option(case_sensitive=False)] = [7]'
        in text
    )
