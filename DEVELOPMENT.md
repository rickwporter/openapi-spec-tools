...
This project uses Python as the programming language, and [uv](https://docs.astral.sh/uv/) for managing packages.

Install uv, then install the `openapi-spec-tools` package with all the dependencies (including the development tools).
...
`uv sync` creates a `.venv` in the project and installs the locked dependencies, including the development group. Your development environment is then ready for coding and testing.
...
```terminal
% TEST_TARGET=tests/cli_gen/test_console.py make cov
uv run coverage run -m pytest -v tests/cli_gen/test_console.py
=============== test session starts ===============
platform darwin -- Python 3.10.18, pytest-8.4.0, pluggy-1.5.0 -- /Users/rick/temp/openapi-spec-tools/.venv/bin/python
